...
@push('links')
    <title>categories-ABS-TECHNOLOGIE</title>
    @section('description','Retrouvez la liste des categories inscrits et gérez leurs informations, spécialisations.')
    <!-- Row Group CSS -->
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">Mes categories</h2>
                    <div class="breadcrumb-wrapper">
                        <x-breadcrumb :links="[
                            ['title' => 'Tableau de bord', 'url' => route('dashboardVendors')],
                            ['title' => 'Paramètres'],
                            ['title' => 'Mes categories', 'url' => route('all.category')]
                        ]" />
                    </div>
                </div>
            </div>
            ...
            <div class="mt-2 mb-5">
                ...
                @include('backend.category.modal.default')
            </div>
            @endcan
        </div>
                ...
                <table class="datatables-basic table" id="declaration-table">
                    <thead>
                        <tr style="background-color: #f3f2f7">
                            ...
                            <th>Description</th>
                            <th>Status</th>
                            @canany(['edit category', 'delete category', 'activate category', 'deactivate category', 'view categories'])
                            <th>Action</th>
                            @endcanany
                        </tr>
                    </thead>
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        @can('deactivate category')
                                            @if($category->status == 1)
                                                <a class="dropdown-item desactive-category-btn" href="#" data-category-id="{{ $category->id }}">
                                        ...
                                        @can('activate category')
                                            @if($category->status == 0)
                                                <a class="dropdown-item active-category-btn" href="#" data-category-id="{{ $category->id }}">
                                        ...
                                        @can('view categories')
                                        <a class="dropdown-item details-category-btn" href="javascript:void(0);" data-category-details-id="{{ $category->id }}">
                                            <i class="ti ti-eye me-1"></i> Détail
                                        </a>
                                        @endcan

                                        @can('edit category')
                                        <a class="dropdown-item" href="{{ route('edit.category', ['id' => $category->id]) }}" data-bs-toggle="modal" data-bs-target="#Modifiercategory">
                                        ...
                                        <a class="dropdown-item text-danger delete-category-btn" href="#" data-category-id="{{ $category->id }}">
...
@push('scripts')
    ...
    @include('backend.category.js.default')
