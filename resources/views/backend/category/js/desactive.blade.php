<script>
    ...
    $(document).on('click', '.desactive-category-btn', function (e) {
        e.preventDefault();

        const categoryId = $(this).data('category-id'); // ID du rôle
        const csrfToken = $('meta[name="csrf-token"]').attr('content'); // CSRF Token

        $.ajax({
            url: `/category/inactive/${categoryId}`, // URL du contrôle de suppression
