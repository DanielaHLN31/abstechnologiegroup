<script type="text/javascript">
    ...
    $(document).on('click', '.delete-product-btn', function (e) {
        ...
        const roleId = $(this).data('product-id'); // ID du rôle
        const csrfToken = $('meta[name="csrf-token"]').attr('content'); // CSRF Token

        // SweetAlert pour la confirmation
        Swal.fire({
            title: 'Êtes-vous sûr ?',
            text: "La suppression de l'product sera définitive !",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: `/products/delete/${roleId}`, // URL du contrôle de suppression
                    ...
                    success: function (response) {
                        if (response.success) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Supprimé!',
                                text: response.message || 'L\'product a été supprimé avec succès.',
                                customClass: {
