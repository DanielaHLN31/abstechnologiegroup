...
$(function () {
  ...
  // -----------------------------------------------------------------------------------------------------------------
    ...
    $('.custom-page .form-repeater').remove();
    ...
    if (formRepeater.length) {
        ...
        formRepeater.repeater({
            ...
            hide: function (e) {
                e.preventDefault();
                const elementToDelete = $(this);

                Swal.fire({
                    title: 'Êtes-vous sûr ?',
                    text: "La suppression du champ sera définitif !",
                    icon: 'warning',
    ...
    // Initialisation du répétiteur pour Unidirectionnel
    $('.repeater-credit-card').repeater({
        show: function () {
            // Gestion spécifique pour ce répétiteur
            var fromControl = $(this).find('.form-control, .form-select');
            var formLabel = $(this).find('.form-label');
            var row = $('.repeater-credit-card').find('[data-repeater-item]').length + 1;

            fromControl.each(function (i) {
                var id = 'form-credit-card-' + row + '-' + i;
                $(fromControl[i]).attr('id', id);
                $(formLabel[i]).attr('for', id);
            });
        ...
        hide: function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    elementToDelete.slideUp(function() {
